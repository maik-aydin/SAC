(function () {
    const depths = 4; // Number of depth levels
    ...
    class SankeyChartStylingPanel extends HTMLElement {
      constructor() {
        ...
        Array.from({ length: depths }, (_, index) => `depth${index}`).forEach((depthId, index) => {
          ["itemColor", "lineOpacity", "textColor", "lineColor"].forEach((setting) => {
      ...
      updateFields() {
        Array.from({ length: depths }, (_, index) => `depth${index}`).forEach((depthId) => {
          ...
          lineOpacityInput.value = settings.lineOpacity !== undefined ? settings.lineOpacity : lineOpacityInput.value || defaultSettings.lineOpacity;
